...
export class FLoWSBeautifier {
  ...
  public flowsBeautify(input_text: string) {
    let out: string = input_text;
    try {
      let c = this.beautify(input_text, this.options);
      if (typeof c !== 'undefined') {
        out = c;
      }
    } catch (error) {
      // if this code fails, at least returns original text.
    }
    return out;
  }
  ...
  /**
   * JS Beautifier was slightly modified to build FLoWS beautifier
   */
  ...
  private nonASCIIwhitespace: RegExp = /[\u1680\u180e\u2000-\u200a\u202f\u205f\u3000\ufeff]/;
  ...
  private beautify(js_source_text: string, options: BeautifierOptions) {
    let output_lines: any[];
    ...
    const flag_store: any[] = [];
    let parser_pos: number = 0;
    let prefix;
    let input_wanted_newline: boolean;
    let output_space_before_token: boolean;
    let input_length: number;
    let n_newlines;
    let whitespace_before_token: any[];
    let opt: { brace_style?: any; indent_size?: any; indent_char?: any; preserve_newlines?: any; break_chained_methods?: any; max_preserve_newlines?: any; spaceIn_paren?: any; spaceIn_empty_paren?: any; jslint_happy?: any; keep_arrayIndentation?: any; space_before_conditional?: any; unescape_strings?: any; wrap_line_length?: any; e4x?: any; };
    ...
    opt.indent_size = options.indent_size ? parseInt(String(options.indent_size), 10) : 4;
    opt.indent_char = options.indent_char ? options.indent_char : ' ';
    opt.preserve_newlines = (options.preserve_newlines === undefined) ? true : options.preserve_newlines;
    opt.break_chained_methods = (options.break_chained_methods === undefined) ? false : options.break_chained_methods;
    opt.max_preserve_newlines = (options.max_preserve_newlines === undefined) ? 0 : parseInt(String(options.max_preserve_newlines), 10);
    ...
    opt.wrap_line_length = (options.wrap_line_length === undefined) ? 0 : parseInt(String(options.wrap_line_length), 10);
    ...
    const input = js_source_text;
    // cache the source's length.
    input_length = js_source_text.length;

    let last_type = 'TK_START_BLOCK'; // last token type
    ...
    const trimOutput = (eat_newlines?: boolean | undefined) => {
    ...
    const trimOutput_line = (line: { text: string[] | void[]; }) => {
    ...
    const justAdded_newline = () => {
      const line = output_lines[output_lines.length - 1];
      return line.text.length === 0;
    }

    const justAdded_blankline = () => {
      if (justAdded_newline()) {
        if (output_lines.length === 1) {
          return true; // start of the file and newline = blank
        }

        const line = output_lines[output_lines.length - 2];
        return line.text.length === 0;
      }
      return false;
    }

    const allow_wrap_or_preserved_newline = (force_linewrap?: boolean | undefined) => {
    ...
    const print_newline = (force_newline?: boolean | undefined, preserve_statement_flags?: boolean | undefined) => {
      output_space_before_token = false;
    ...
    const print_token_lineIndentation = () => {
      if (justAdded_newline()) {
        const line = output_lines[output_lines.length - 1];
    ...
    const printIndent_string = (level: number) => {
    ...
    let print_token_space_before = () => {
      const line = output_lines[output_lines.length - 1];
      if (output_space_before_token && line.text.length) {
        const lastOutput = line.text[line.text.length - 1];
    ...
    const print_token = (printable_token?: string | any[] | undefined) => {
    ...
    const remove_redundantIndentation = (frame: any) => {
      ...
      let index = frame.start_lineIndex;
      let spliceIndex = 0;
      let line;
    ...
    const is_array = (mode: any) => {
      return mode === MODE.ArrayLiteral;
    }

    const is_expression = (mode: any) => {
    ...
    const start_of_object_property = () => {
      return flags.parent.mode === MODE.ObjectLiteral
        && flags.mode === MODE.Statement
        && flags.last_text === ':'
        && flags.ternary_depth === 0;
    }

    let start_of_statement = () => {
      if (
    ...
    const all_lines_start_with = (lines: string | any[], c: string) => {
    ...
    const each_line_matchesIndent = (lines: string | any[], indent: any) => {
      let i = 0;
    ...
    const is_special_word = (word: any) => {
      return in_array(word, ['case', 'return', 'do', 'if', 'throw', 'else']);
    }

    const in_array = (what: string | any[], arr: string | any[]) => {
    ...
    const unescape_string = (s: string) => {
      let esc = false;
      let out = '';
      let pos = 0;
      let s_hex = '';
    ...
    const get_next_token = () => {
      ...
      if (this.isIdentifierChar(input.charCodeAt(parser_pos - 1))) {
        if (parser_pos < input_length) {
          while (this.isIdentifierChar(input.charCodeAt(parser_pos))) {
            c += input.charAt(parser_pos);
            parser_pos += 1;
            if (parser_pos === input_length) {
              break;
      ...
      if (c === '`' || c === '\'' || c === '"' || // string
        ...
        if (parser_pos < input_length) {
          if (sep === '/') {
            ...
            const xmlStr = input.slice(parser_pos - 1);
    ...
    const handle_block_comment = () => {
      const lines = split_newlines(token_text);
      let j; // iterator for this case
      let javadoc = false;
      let starless = false;
      const lastIndent = whitespace_before_token.join('');
