...
export class FLoWSBeautifier {
  ...
  private beautify(js_source_text, options) {
    ...
    let allow_wrap_or_preserved_newline = (force_linewrap?) => {
    ...
    let print_newline = (force_newline?, preserve_statement_flags?) => {
      output_space_before_token = false;
    ...
    let print_token_lineIndentation = () => {
      if (justAdded_newline()) {
        var line = output_lines[output_lines.length - 1];
    ...
    let print_token = (printable_token?) => {
    ...
    let remove_redundantIndentation = (frame) => {
    ...
    let all_lines_start_with = (lines, c) => {
    ...
    let each_line_matchesIndent = (lines, indent) => {
      var i = 0,
        len = lines.length,
        line;
      for (; i < len; i++) {
    ...
    let is_special_word = (word) => {
      return in_array(word, ['case', 'return', 'do', 'if', 'throw', 'else']);
    }

    let in_array = (what, arr) => {
